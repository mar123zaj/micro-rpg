...
export default class ReferenceScene extends Phaser.Scene {
  ...
  previewTileset(): void {
    ...
    const layer = this.map.createBlankLayer('preview', tiles, 30, 40, tiles.columns, tiles.rows);
    ...
    this.title!.setText(`'${tileset.name}' (${this.index + 1} of ${tilesets.length}) ['n' for next]`);
  }
}
