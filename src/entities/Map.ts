  ...
  constructor(width: number, height: number, scene: DungeonScene) {
    ...
    for (const room of dungeon.rooms) {
      ...
      for (let i = 0; i < numSlimes; i++) {
        this.slimes.push(
          new Slime(Phaser.Math.Between(roomTL.x, roomBounds.x), Phaser.Math.Between(roomTL.y, roomBounds.y), scene),
        );
      }
    }
    ...
    this.doorLayer.setTileIndexCallback(
      ...
      (_: unknown, tile: Phaser.Tilemaps.Tile) => {
        this.doorLayer.putTileAt(Graphics.environment.indices.doors.destroyed, tile.x, tile.y);
  ...
  withinRoom(x: number, y: number): boolean {
    return (
      this.rooms.find((r) => {
        ...
        return y >= top - 1 && y <= bottom + 1 && x >= left - 1 && x <= right + 1;
      }) != undefined
    );
  }
}
