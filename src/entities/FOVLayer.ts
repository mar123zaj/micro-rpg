...
function updateTileAlpha(desiredAlpha: number, delta: number, tile: Phaser.Tilemaps.Tile): void {
...
export default class FOVLayer {
  ...
  constructor(map: Map) {
    ...
    this.layer = map.tilemap.createBlankLayer('Dark', utilTiles, 0, 0).fill(Graphics.util.indices.black);
  ...
  recalculate(): void {
    this.mrpas = new Mrpas(this.map.width, this.map.height, (x: number, y: number) => {
      return this.map.tiles[y] && !this.map.tiles[y][x].collides;
    });
  }

  update(pos: Phaser.Math.Vector2, bounds: Phaser.Geom.Rectangle, delta: number): void {
  ...
  updateMRPAS(pos: Phaser.Math.Vector2): void {
    // TODO: performance?
